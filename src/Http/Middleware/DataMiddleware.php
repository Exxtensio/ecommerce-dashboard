...
class DataMiddleware extends Middleware
{
    ...
    public function share(Request $request): array
    {
        ...
        if(class_exists(InstalledVersions::class) && InstalledVersions::isInstalled('exxtensio/ecommerce-dashboard')) {
            ...
            if(!Cache::has('sellexx.dashboard.version.releases')) {
                ...
                Cache::put('sellexx.dashboard.version.releases', [
                    ...
                    'uninstalledList' => $latestReleases->toArray(),
                ], now()->addHour());
            }
        }


        if (!Cache::has('sellexx.dashboard.version.current') && class_exists(InstalledVersions::class) && InstalledVersions::isInstalled('exxtensio/ecommerce-dashboard')) {
            $currentVersion = InstalledVersions::getPrettyVersion('exxtensio/ecommerce-dashboard');
            Cache::put('sellexx.dashboard.version.current', $currentVersion, now()->addHour());
        }
