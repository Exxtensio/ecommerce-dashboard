...
use Exxtensio\EcommerceDashboard\Models\Language;
...
class DataMiddleware extends Middleware
{
    ...
    public function share(Request $request): array
    {
        ...
        if(config('dashboard.only_translations') && is_array(config('dashboard.only_translations'))) {
            $appLanguages = Language::whereIn('code', config('dashboard.only_translations'))
                ->select(['name_local as name', 'code as value'])
                ->get();
        } else {
            $appLanguages = Language::select(['name_local as name', 'code as value'])->get();
        }
