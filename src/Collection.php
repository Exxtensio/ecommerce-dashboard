...
class Collection extends ResourceCollection
{
    ...
    private function getCollectionRelations($newResource, $fields): array
    {
        ...
        return collect($newResource->withOptions)->mapWithKeys(function ($m, $r) use ($newResource, $except) {
            ...
            return [
                $relatedResource::$prefix => (new $relatedResource::$model())::when($except, function ($query) use ($except) {
                    ...
                    ->map(function ($item) use ($relatedResource) {
                        if ($relatedResource::$prefix === 'permissions') {
                            ...
                            $flagSVG = strtolower($item->code).'.svg';
                            $flagPath = "vendor-dashboard-data/flags/4x3/$flagSVG";
                            return [
                                'name' => $item[$relatedResource::$title],
                                'code' => $item->code,
                                'flag' => file_exists(public_path($flagPath)) ? asset($flagPath) : null,
                                'value' => $item->id,
                                'active' => $item->active,
                                'relations' => [
