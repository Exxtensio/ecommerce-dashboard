...
class Guard
{
    ...
    {
        $class = is_object($model)
            ? get_class($model)
            : $model;

        if (is_object($model))
            $guardName = method_exists($model, 'guardName')
                ? $model->guardName()
                : $model->getAttributeValue('guard_name');

        if (!isset($guardName))
            $guardName = (new ReflectionClass($class))
                ->getDefaultProperties()['guard_name'] ?? null;
