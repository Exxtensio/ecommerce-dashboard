<template>
    ...
    <nav class="fixed top-0 z-40 w-full border-b theme-brand-bg theme-brand-border-color">
        <div class="px-3 py-3 lg:px-5 lg:pl-3 shadow-sm dark:shadow-none">
    ...
    <div class="flex flex-col mt-16" :class="{'md:ml-[4.4rem]': minimize, 'md:ml-64': !minimize}" style="min-height: calc(100vh - 65px);">
        <slot/>
        <div class="mt-auto text-gray-400 dark:text-gray-500 px-6 pb-3 text-xs flex justify-between max-[520px]:flex-col max-[520px]:space-y-1">
            <p>Copyright © 2022-{{ new Date().getFullYear() }} Exxtensio LLC, All rights reserved.</p>
...
export default {
    ...
    mounted() {
        ...
        setTimeout(function () {
            if (this.$globalStore.access && this.$globalStore.access.errors.length) {
                this.emitter.emit('notify', {
                    position: 'top-center',
                    type: 'danger',
                    duration: 9999999,
                    message: this.$globalStore.access.errors
                })
            }
        }.bind(this), 500)
    },
    ...
    computed: {
        ...
        currentVersion() {
            return localStorage.getItem('sellexx_version')
        }
    }
}
</script>
