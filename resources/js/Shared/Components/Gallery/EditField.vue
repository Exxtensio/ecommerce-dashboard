    ...
    <div :class="{'required': isRequired, 'error': localErrorMessage}">
        ...
        <div v-if="field.resolvedForDisplay.length" class="grid gap-2 grid-cols-3 mt-4">
            <div v-for="(file, index) in field.resolvedForDisplay.filter(f => !f.deleted)" class="group relative">
                ...
                <button
                    v-if="!file.default || field.resolvedForDisplay.filter(f => !f.deleted).length === 1"
        ...
        <div v-if="localErrorMessage && localErrorMessage.length">
            <exx-help-validation v-for="error in localErrorMessage" :text="error" type="text"/>
        </div>
    </div>
</template>
...
export default {
    ...
    watch: {
        errors: {
            ...
            immediate: true,
            handler(newErrors) {
                this.localErrorMessage = newErrors && Object.keys(newErrors).some(key => key.startsWith(`${this.field.attribute}.`))
                    ? Object.values(newErrors).flat()
                    : null
            },
        },
    },
