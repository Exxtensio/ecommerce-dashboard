<template>
    <div :class="{'required': isRequired}" class="readonly">
        ...
        <div v-if="field.resolvedForUpdate.length" class="text-sm text-right max-w-[300px] ml-auto mt-[5px]">
            <div class="py-2">Total Items: {{ field.resolvedForUpdate.length }}</div>
            <div class="py-2 border-t theme-brand-border-color">Total Quantity: {{ totalQuantity }}</div>
            <div class="py-2 border-t theme-brand-border-color">Total Price: {{ totalSum }}</div>
        </div>
    </div>
</template>
