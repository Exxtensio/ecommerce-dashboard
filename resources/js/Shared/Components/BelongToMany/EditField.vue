<template>
    <div :class="{'required': isRequired}">
        ...
        <div
            ...
            :class="{
                'gap-2 grid-cols-3': needGroupingByRelation && list.length,
                'gap-4 grid-cols-2': needGroupingByRelation && (groupedList.length || ungroupedList.length),
                'gap-4 grid-cols-3': needGroupingByKey
            }"
        >
            ...
            <template v-else-if="needGroupingByRelation && (groupedList.length || ungroupedList.length)">
                <div v-for="group in groupedList" class="border theme-brand-border-color rounded-md p-4">
                    ...
                    <fwb-checkbox
                        ...
                        @update:modelValue="(checked) => setChecked(group, checked)"
                        ...
                        <div v-for="child in group.children" class="grid">
                            ...
                            <template v-else>
                                <fwb-checkbox
                                    ...
                                    @update:modelValue="(checked) => setChecked(child, checked)"
                                    class="mb-2 truncate"
                                />
        ...
        <p v-else class="text-sm py-2">There is no {{ field.attribute }}</p>
    </div>
</template>
...
export default {
    ...
    watch: {
        combinedList: {
            ...
            handler(value) {
                let result = []
                result = result.concat(this.collectAll(value.list))
                result = result.concat(this.collectAll(value.groupedList))
                result = result.concat(this.collectAll(value.ungroupedList))
                this.field.resolvedForUpdate = result
            }
        }
    },
    created() {
    ...
    methods: {
        ...
        setChecked(category, isChecked) {
            category.checked = isChecked
            if (category.children && category.children.length > 0)
                category.children.forEach(child => this.setChecked(child, isChecked))
        },
        collectAll(items) {
            let result = []
            items.map(item => {
                if (item.hasOwnProperty('checked') && item.checked) result.push(item.value)
                if (item.hasOwnProperty('items') && item.items.length) {
                    result = result.concat(this.collectAll(item.items))
                } else if (item.hasOwnProperty('children') && item.children.length) {
                    result = result.concat(this.collectAll(item.children))
                }
            })
            return result
        },
    ...
    computed: {
        ...
        combinedList() {
            if (this.needGroupingByRelation && this.list.length)
                return {list: this.list, groupedList: [], ungroupedList: []}
            if (this.needGroupingByRelation && (this.groupedList.length || this.ungroupedList.length))
                return {list: [], groupedList: this.groupedList, ungroupedList: this.ungroupedList}
            return {list: this.list, groupedList: [], ungroupedList: []}
        },
        length() {
