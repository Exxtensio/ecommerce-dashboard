<template>
    <div v-if="hasSoftDelete" class="flex items-center space-x-3 relative" v-click-away="onClickAway">
        <button
            @click="open = !open"
        ...
        >
            <TrashIcon class="size-4 -ml-1 mr-2 max-2xl:mr-0"/>
            <span class="max-2xl:hidden">{{ $t('Trashed') }}</span>
        ...
        <div class="z-10 min-w-44 absolute right-0 top-[43px] theme-brand-dropdown !divide-y-0 max-lg:right-auto max-lg:left-0" :class="{'hidden': !open}">
            <div class="px-3 pt-3">
                <h6 class="mb-2 small-head-label">{{ $t('Options') }}</h6>
                <ul class="space-y-2">
