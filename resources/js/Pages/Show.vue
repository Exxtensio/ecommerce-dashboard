                ...
                <div class="mb-6 mt-10 flex items-center justify-between max-[1320px]:flex-col max-[1320px]:items-baseline max-[1320px]:space-y-4">
                    <div class="text-white space-y-2">
                        <div class="flex items-end space-x-4">
                            ...
                            <div v-if="can(getId, 'canEdit') && !getLocked">
                                <fwb-button size="xs" color="light" class="btn-light" @click="$router.visit(getResourceUrl(getId + '/edit'))">{{ $t('Edit') }}</fwb-button>
                            </div>
                        </div>
                    ...
                    <div v-for="(name, key) in uniquePanels" v-show="currentPanel === key" class="grid grid-cols-2 gap-6 items-baseline max-[1180px]:grid-cols-1">
                        <template v-for="field in fields.filter(f => f.panel === key)">
                            <component
                                ...
                                class="component detail-component max-[1180px]:col-auto"
...
export default {
    ...
    mounted() {
        this.$echo.channel(`eloquent.${this.prefix}.${this.getId}`)
            .listen('.locked', (e) => {
                if(this.auth.id !== e.userId)
                    this.fields.filter(i => i.component === 'id-field')[0].locked = true
            })
            .listen('.unlocked', (e) => {
                this.fields.filter(i => i.component === 'id-field')[0].locked = false
            })
    },
    unmounted() {
        this.$echo.leaveChannel(`eloquent.${this.prefix}.${this.getId}`)
    },
    computed: {
        uniquePanels() {
        ...
        getLocked() {
            return this.fields.filter(i => i.component === 'id-field')[0].locked
        },
        editLink() {
