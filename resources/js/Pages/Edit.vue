                    ...
                    <div class="text-white space-y-2">
                        <div class="flex items-end space-x-4">
                            ...
                            <div v-if="can(getId, 'canPreview')">
                                <fwb-button size="xs" color="light" class="btn-light" @click="$router.visit(getResourceUrl(getId))">{{ $t('Preview') }}</fwb-button>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        async update(redirect) {
            ...
            await this.$axios.post(this.getResourceUrl(this.getId), this.dataToUpdate)
                .then(response => {
    ...
    beforeUnmount() {
        this.$axios.post(this.getResourceUrl('r'), {
            id: this.getId
        })
    },
    computed: {
