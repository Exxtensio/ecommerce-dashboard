            ...
            <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-900 dark:border-gray-700">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <div class="space-y-4 md:space-y-6">
                        ...
                        <fwb-button color="default" size="md" @click="login" class="w-full">{{ $t('Sign in') }}</fwb-button>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            {{ $t('Don\'t have an account yet?') }} <a class="btn-link hover:underline" href="/register">{{ $t('Sign up') }}</a>
                        </p>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        async login() {
            this.errors = {}
            await axios.post(this.getUrl('login'), {
                email: this.email,
