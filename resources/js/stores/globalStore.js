...
export const useGlobalStore = defineStore('sellexx_global', {
    ...
    actions: {
        ...
        verifyKey(key, secret) {
            ...
            let warns = [];
            if(!secret || !key) {
                if(!key) errors.push(this.$t('SELLEXX_DASHBOARD_KEY was not found in the .env file.'))
                if(!secret) errors.push(this.$t('SELLEXX_DASHBOARD_SECRET was not found in the .env file.'))
                    ...
                    axios.post(`https://exxtensio.com/api/licence/${key}`, {}, {headers: {'X-Api-Key': 'YCwWdyr7JNfZM92mh4zQGxDsVFHk6n5aebpEvB8LA3RucKUtPT'}})
                        .then((response) => {
                            ...
                            if(window.location.origin !== realData.host || window.location.origin !== responseData.host || realData.host !== responseData.host)
                                errors.push(
                                    this.$t('The used host :originHost does not match the expected host :responseHost.', {originHost: window.location.origin, responseHost: responseData.host})
                                )

                            if(realData.type !== responseData.type)
                                errors.push(this.$t('The type of the used license is incorrect.'))

                            if(currentTimestamp > realData.expired_at || currentTimestamp > responseData.expired_at)
                                errors.push(this.$t('Your license has expired.'))
                ...
                if(left === 0 || left < 0) {
                    ...
                    if(daysLeft < 2) {
                        warns.push(this.$t('Your license will expire in less than 1 day.'))
                    } else if (daysLeft < 4) {
                        warns.push(this.$t('Your license will expire in less than 3 days.'))
                    } else if (daysLeft < 8) {
                        warns.push(this.$t('Your license will expire in less than 7 days.'))
                    }
                    this.setWarns(warns)
                }
            }
